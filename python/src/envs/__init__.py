...
def smaclite_fn(**kwargs) -> MultiAgentEnv:
    # Lazy import to avoid dependency unless explicitly used
    from .smaclite_wrapper import SMACliteWrapper  # type: ignore
...
def gymma_fn(**kwargs) -> MultiAgentEnv:
    assert "common_reward" in kwargs and "reward_scalarisation" in kwargs
    # Lazy import to avoid gymnasium dependency when not used
    from .gymma import GymmaWrapper  # type: ignore
...
REGISTRY["quicfec"] = lambda **kwargs: QuicFecWrapper(**kwargs)
