...
class QuicFecRunner:
    ...
    def _sudo_ensure(self) -> None:
        ...
        # The shell harness supports a narrowly scoped root-owned helper. The
        # Python runner must accept the same mechanism instead of requiring a
        # direct sudo ticket before constructing FecEnv.
        helper = os.environ.get("QUIC_FEC_PRIV_HELPER", "").strip()
        if helper:
            try:
                subprocess.run(["sudo", "-n", helper, "self-test"], check=True, capture_output=True)
                return
...
class FecEnv(gym.Env):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # Order: [K_idx, R0_idx, RSTEP_idx]. DDL is generated by the sender
        # from the QUIC congestion controller and carried in the data header.
        #   K:      20..60 step 2    -> len(self._k_values)
        #   R0:      0..20 step 2    -> len(self._r0_values)
        #   RSTEP:   0..20 step 2    -> len(self._rstep_values)
        self.action_space = spaces.MultiDiscrete(
            [int(len(self._k_values)), int(len(self._r0_values)), int(len(self._rstep_values))]
        )
