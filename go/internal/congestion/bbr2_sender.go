...
const (
	...
	bbrv2MinCongestionWindowPackets = 4
...
type BBR2Sender struct {
	...
	lastDebugPacing Bandwidth
}

// Compile-time check that BBR2Sender implements CongestionControlEx.
var _ CongestionControlEx = (*BBR2Sender)(nil)
...
func newBBR2Sender(
	...
	return newBBR2SenderWithInitialRTT(
		clock,
		maxDatagramSize,
...
func newBBR2SenderWithInitialRTT(
	clock Clock,
	maxDatagramSize ByteCount,
	initialCongestionWindow ByteCount,
	aggressive bool,
	...
	if cwnd == 0 {
		cwnd = ByteCount(InitialCongestionWindowPackets) * maxDatagramSize
	}

	maxCwnd := ByteCount(MaxCongestionWindowPackets) * maxDatagramSize
...
func initialPacingRate(cwnd ByteCount, rtt time.Duration, params *Params) Bandwidth {
...
func (s *BBR2Sender) SetRTTStatsProvider(provider RTTStatsProvider) {
	s.rttStats = provider
}

// TimeUntilSend returns when the next packet should be sent.
func (s *BBR2Sender) TimeUntilSend(bytesInFlight ByteCount) time.Time {
...
func (s *BBR2Sender) HasPacingBudget(now time.Time) bool {
...
func (s *BBR2Sender) CanSend(bytesInFlight ByteCount) bool {
...
func (s *BBR2Sender) OnCongestionEventEx(
	...
	s.model.OnCongestionEventStart(ackedPackets, lostPackets, ecnCE, congestionEvent)
	...
	s.maybeDebugEstimate(eventTime)

	// Estimate leastUnacked for cleanup (same approach as BBRv1)
	var leastUnacked PacketNumber
...
func (s *BBR2Sender) GetCongestionWindow() ByteCount {
	return s.cwnd
}

// SetMaxDatagramSize updates the max datagram size.
func (s *BBR2Sender) SetMaxDatagramSize(size ByteCount) {
	...
	s.cwnd = ByteCount(float64(s.cwnd) * factor)
...
func (s *BBR2Sender) maybeDebugEstimate(now time.Time) {
	if s.tracer == nil || s.tracer.Debug == nil {
		return
	}
	if !s.lastDebugTime.IsZero() && now.Sub(s.lastDebugTime) < 200*time.Millisecond {
		return
	}
	pacing := s.PacingRate()
	if pacing == s.lastDebugPacing && !s.lastDebugTime.IsZero() {
		return
	}
	s.lastDebugTime = now
	s.lastDebugPacing = pacing

	bwMbps := float64(s.BandwidthEstimate()) / 1e6
	pacingMbps := float64(pacing) / 1e6
	if s.BandwidthEstimate().IsInfinite() {
		bwMbps = 0
	}
	if pacing.IsInfinite() {
		pacingMbps = 0
	}
	s.tracer.Debug("cc-estimate", fmt.Sprintf(
		"{\"algo\":\"bbrv2\",\"mode\":\"%s\",\"method\":\"bbrv2\",\"bw_mbps\":%.6f,\"pacing_bw_mbps\":%.6f,\"cwnd_bytes\":%d}",
		s.mode.String(), bwMbps, pacingMbps, s.cwnd,
	))
}
...
func (s *BBR2Sender) getTargetCongestionWindow(gain float64) ByteCount {
	target := s.model.BDP(s.model.BandwidthEstimate(), gain)
	minTarget := ByteCount(s.cwndLimits.Lo)
	if target < minTarget {
		return minTarget
	}
	return target
...
func (s *BBR2Sender) targetBytesInflight() ByteCount {
...
func (s *BBR2Sender) updatePacingRate(bytesAcked ByteCount) {
...
func (s *BBR2Sender) updateCongestionWindow(bytesAcked ByteCount) {
	...
	s.cwnd = ByteCount(modeLimits.ApplyLimits(int(s.cwnd)))

	// Apply global limits
	s.cwnd = ByteCount(s.cwndLimits.ApplyLimits(int(s.cwnd)))
}

// onExitQuiescence handles exiting quiescence.
func (s *BBR2Sender) onExitQuiescence(now time.Time) {
...
func (s *BBR2Sender) onEnterQuiescence(now time.Time) {
	s.lastQuiescenceStart = &now
}

// OnPacketNeutered handles a neutered packet.
func (s *BBR2Sender) OnPacketNeutered(packetNumber PacketNumber) {
	s.model.OnPacketNeutered(packetNumber)
}

// OnAppLimited marks the connection as app limited.
func (s *BBR2Sender) OnAppLimited(bytesInFlight ByteCount) {
	if bytesInFlight >= s.GetCongestionWindow() {
		return
	}
...
func (s *BBR2Sender) OnPacketsLost(leastUnacked PacketNumber) {
