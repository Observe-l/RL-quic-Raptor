...
const (
	...
	invalidPacketNumber                = PacketNumber(0) - 1
)
...
func (r *recentAckPoints) update(ackTime time.Time, totalBytesAcked ByteCount) {
...
type extraAckedSample struct {
	extraAcked ByteCount
	bytesAcked ByteCount     // Total bytes acked in this aggregation epoch
...
func (f *extraAckedFilter) update(extraAcked, bytesAcked ByteCount, timeDelta time.Duration, round RoundTripCount) {
	s := &extraAckedSample{
...
type maxAckHeightTracker struct {
	...
	aggregationEpochBytes                  ByteCount
	lastSentPacketNumberBeforeEpoch        PacketNumber
...
func (t *maxAckHeightTracker) update(
	...
	lastSentPacketNumber PacketNumber,
	lastAckedPacketNumber PacketNumber,
	ackTime time.Time,
	...
	if t.aggregationEpochBytes <= ByteCount(t.ackAggregationBandwidthThreshold*float64(expectedBytesAcked)) {
		t.aggregationEpochBytes = bytesAcked
		t.aggregationEpochStartTime = ackTime
		t.lastSentPacketNumberBeforeEpoch = lastSentPacketNumber
		t.numAckAggregationEpochs++
...
func (t *maxAckHeightTracker) maxAckHeight() ByteCount {
	best := t.maxAckHeightFilter.getBest()
	if best == nil {
		return 0
	}
...
type connectionStateMap struct {
	packets map[PacketNumber]*connectionStateOnSentPacket
}

func newConnectionStateMap() *connectionStateMap {
	return &connectionStateMap{
		packets: make(map[PacketNumber]*connectionStateOnSentPacket),
	}
}

func (m *connectionStateMap) insert(pktNum PacketNumber, state *connectionStateOnSentPacket) {
	m.packets[pktNum] = state
}

func (m *connectionStateMap) take(pktNum PacketNumber) *connectionStateOnSentPacket {
...
func (m *connectionStateMap) removeObsolete(leastAcked PacketNumber) {
...
func (s *BandwidthSampler) OnPacketNeutered(packetNumber PacketNumber) {
...
func (s *BandwidthSampler) OnCongestionEvent(
	...
	var lastLostPacketNum PacketNumber
	var lastAckedPacketNum PacketNumber
...
func (s *BandwidthSampler) onPacketLost(packetNumber PacketNumber, bytesLost ByteCount) SendTimeState {
...
func (s *BandwidthSampler) chooseA0Point(totalBytesAcked ByteCount) *ackPoint {
...
func (s *BandwidthSampler) TotalBytesAcked() ByteCount {
	return s.totalBytesAcked
}

// TotalBytesLost returns the total bytes lost.
func (s *BandwidthSampler) TotalBytesLost() ByteCount {
	return s.totalBytesLost
}

// MaxAckHeight returns the maximum ack height.
func (s *BandwidthSampler) MaxAckHeight() ByteCount {
...
func (s *BandwidthSampler) RemoveObsoletePackets(leastAcked PacketNumber) {
